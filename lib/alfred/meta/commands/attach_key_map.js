...
AttachKeyMapCommand.prototype.do = function(meta, callback) {
  ...
  key_map_type.open(key_map_path, function(err, key_map) {
    if (err) {
      ...
      // clear should clear search indexes as well
      key_map._clear = key_map.clear;
      key_map.clear = function (callback) {
        this._clear(function (err) {
          for(var index_index in this.indexes) {
    	    if (this.indexes.hasOwnProperty(index_index)) {
    	      index = this.indexes[index_index];
    	      index.clear();
    	    }
    	  }
    	  callback(err);
