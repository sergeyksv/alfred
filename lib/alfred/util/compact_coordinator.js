...
CompactCoordinator.prototype.copyIntoCollectionAndIndexes = function(collection, indexes, callback) {
  ...
  index_temp_temp.each(function(err, key, pos, length) {
    copyKeys.push(arguments);
    copied_count ++;
    if (copied_count == count)
      // copy elemts in serialized (sequentional fashon)
      async.forEachSeries(copyKeys, function (item, callback) {
        var key = item[1], pos=item[2], length=item[3];
        doSingleCopy(key, pos, length, callback);
      },callback)
  })
  
  function doSingleCopy(key,pos,length,callback) {
        ...
        self.key_map.getAtPos(pos, length, function(err, key, value) {
          ...
          self.key_map.putAtPos(key, value, this_pos, function(err, position, length) {
            if (err) {
              ...
              current_pos += length;
              self.index_temp.put(key, value, position, length, function(err) {
                if (err) {
                  callback(err);
                } else {
            ...
            if (err) {
              ...
              for(var index_index in self.key_map.indexes) {
                if (self.key_map.indexes.hasOwnProperty(index_index)) {
                  ...
                  cloned_index = new (index.ordered ? OrderedIndex : UnorderedIndex).open(index.transform_function,index.options);
