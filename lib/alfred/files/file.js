...
/* Write 
 * Now it need serialization of write operations because
 * accourding to docs multiple fs.write on single file
 * cause unpredicatble result. In our case they cause a mess
 * with actual record positions and corruption of file
 *
 * With funcSerializer help write operation is guaranteed to 
 * be executed one at a time
 */
var FuncSerializerCtx = function(parent, func) {
  var self = this;
  var q = [];
  var inCall = false;

  this.pump = function (newArgs) {
    if (newArgs) q.push(newArgs);
    if (inCall == false && q.length>0) {
      inCall = true;
      var wop = q.shift();
      var pcb = wop[wop.length-1];
      wop[wop.length-1] = function () {
        inCall = false;
        var lthis = this;
        var largs = arguments;
        process.nextTick(function () {
    	    pcb.apply(lthis,largs);
	    self.pump();
	})
      }
      func.apply(parent,wop);
    }
  }
}

function funcSerializer(ctx, func) {
  return function () {
    if (this[ctx] == null)
	this[ctx] = new FuncSerializerCtx(this,func);
    this[ctx].pump(arguments);
  }
}

File.prototype.write = funcSerializer('writeSerializer', function(string, callback) {
